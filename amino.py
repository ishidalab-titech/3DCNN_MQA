...
def remove_notCNOS(mol):
    mol.element = np.array([''] * len(mol.element))
    mol.element = np.array(mol._guessMissingElements())
    c = set([i for i in mol.name if i[0] not in ['C', 'N', 'O', 'S']])
    for name in c:
        mol.remove('name %s' % (name), _logger=False)
    return mol
...
class Atom():
    ...
    def get_type(self, resname, name):
        if ((self.resname == resname) | (resname == 'ALL')) & (self.name == name):
            return True
        else:
            return False


def get_atom_type_array(mol):
    atom_name = mol.get('name')
    amino_name = mol.get('resname')
    channels = []
    for i in range(len(atom_name)):
        if i == len(atom_name) - 1:
            channels.append(atom_type(resname=amino_name[i], name=atom_name[i]))
        else:
            channels.append(atom_type(resname=amino_name[i], name=atom_name[i], C_terminal=True))
